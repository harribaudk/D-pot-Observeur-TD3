...
public class App {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < 10; i++) {
            dataCompression.setStrategy(new ImageStrategy());
            dataCompression.setStrategy(new AudioStrategy());
            dataCompression.setStrategy(new VideoStrategy());

            imageResult = dataCompression.processData(imageList.get(i));
            soundResult = dataCompression.processData(audioList.get(i));
            if (i < 5)
                videoResult = dataCompression.processData(videoList.get(i));
