...
public class App {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < 10; i++) {
            dataCompression.strategie(new ImageStrategy());
            dataCompression.strategie(new AudioStrategy());
            dataCompression.strategie(new VideoStrategy());

            imageResult = dataCompression.compresser(imageList.get(i));
            soundResult = dataCompression.compresser(audioList.get(i));
            if (i < 5)
                videoResult = dataCompression.compresser(videoList.get(i));
